...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    ...
    if tensor_list[0].ndim == 3:
        ...
        # max_size will be [C, H, W]

        padded_imgs = []
        padded_masks = []
        for img in tensor_list:
            # 计算需要填充的量 (右边和下边)
            # F.pad 的 padding 参数格式是 (pad_left, pad_right, pad_top, pad_bottom)
            padding = (0, max_size[2] - img.shape[2], 0, max_size[1] - img.shape[1])

            # 使用 F.pad 进行非原地操作，生成一个新的、填充好的张量
            padded_img = F.pad(img, padding, "constant", 0)
            padded_imgs.append(padded_img)

            # 同时生成对应的 mask
            # 创建一个和原始图像一样大的全1 mask
            mask = torch.zeros((img.shape[1], img.shape[2]), dtype=torch.bool, device=img.device)
            # 对 mask 进行同样的填充，填充值为 True (表示是填充区域)
            padded_mask = F.pad(mask, padding, "constant", True)
            padded_masks.append(padded_mask)

        # 将填充好的张量列表和掩码列表堆叠成一个批次
