...
class BCEWithLogitsLoss(ClassificationLoss):
    ...
    def __call__(self, preds: torch.Tensor, targets: torch.Tensor, valid_mask: torch.Tensor , text_mask: torch.Tensor=None) -> torch.Tensor:
        ...
        # print(targets,targets.shape,"tttttttttttttttttttttttttttttttttttttt")
...
class FocalLoss(ClassificationLoss):
    ...
    def __call__(self, preds: torch.Tensor, targets: torch.Tensor, valid_mask: torch.Tensor, text_mask: torch.Tensor=None) -> torch.Tensor:
        ...
        # new_targets = torch.masked_select(targets, text_mask) * 5
        # print(text_mask[:,0:1,0:15],"qqqqqqqqqqqqqqqqqqqqqqqqqqqqqq")
        new_targets = torch.masked_select(targets, text_mask)  # gt对应[0., 1., 1., 1., 1., 1., 1., 0., 0.,] 非gt对应[0,0,...,0]，一共有900个（查询个数）
        # print(new_targets[:30])
        # print((new_targets == 1).nonzero(as_tuple=True),"nnnnnnnnnnnnnnnnnnnnnnnn")
