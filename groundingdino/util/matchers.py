...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        # print(targets,"nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn")
        ...
        for t in targets:
            ...
            # 直接提取 cat2tokenspan 中的跨度，确保结构正确
            # print(t['str_cls_lst']) # # gt:adversarial patch
            span_list = [t['cat2tokenspan'][cls] for cls in t['str_cls_lst']]
            # print(span_list,"***********************************************")
            token_spans.append(span_list)
        print(token_spans,"666666666666666666666666666666666666666") # [0, 11], [12, 17]]
        ...
        max_length = 256
        # print(outputs['tokenized'],"ddddddddddddddddddddddddddddddddddddddddd")
        ...
        # print((tgt_labels[:, :15]),"yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy")
        # Compute the classification cost. Contrary to the loss, we don't use the NLL,
        # but approximate it in 1 - proba[target class].
        # The 1 is a constant that doesn't change the matching, it can be ommitted.

        normalized_tgt_labels = tgt_labels
        # normalized_tgt_labels = tgt_labels / (tgt_labels.sum(dim=1, keepdim=True) + self.stability_eps)
        # normalized_tgt_labels = normalized_tgt_labels * 5
        # print(normalized_tgt_labels,"?zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz")
        # Compute costs using normalized labels
        # print(out_prob, out_prob.shape,"555555555555555555555555555555555555555555")
        # print(out_prob[:,8:10],"6666666666666666666666666666666666666")
        # print(out_prob.unsqueeze(1).shape, normalized_tgt_labels.unsqueeze(0).shape) # torch.Size([7200, 1, 256]) torch.Size([1, 20, 256])
        cost_class = -(out_prob.unsqueeze(1) * normalized_tgt_labels.unsqueeze(0).float()).sum(dim=-1)  # 二分图匹配过程中计算分类损失
        # print(out_prob.unsqueeze(1),"qqqqqqqqqqqqqqqqqqqqqqqqqq")
        # print(normalized_tgt_labels.unsqueeze(0),"llllllllllllllllllllllllll")
        # print(cost_class,"t`````````````````````````````````````````````````")
        #cost_class = -(out_prob.unsqueeze(1) * tgt_labels.unsqueeze(0).float()).sum(dim=-1) ## shape is [num_queries x num of boxes]
        # print(pred_bbox, pred_bbox.shape, cost_class,"llllllllllllllllllll")
        # print(tgt_bbox, tgt_bbox.shape,"mmmmmmmmmmmmmmmmmm")
        # Compute the L1 cost between boxes
        cost_bbox = torch.cdist(pred_bbox, tgt_bbox, p=1) ## shape is [num_queries x num of boxes]
        # print(cost_bbox,"ccccccccccccccccccccccccccccccccccccccccc")
        ...
        # print(cost_giou,"iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")

        # Final cost matrix
        C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_giou
        C = C.view(bs, num_queries, -1).cpu() # (8, 900, gt_num)

        sizes = [len(v["boxes"]) for v in targets]  # gt_num of each image
        # print(C.split(sizes, -1)) # split C according to sizes
        # for i, a in enumerate(C.split(sizes, -1)):
        #     print(a[i].shape)
        # c.shape is [8, 900, gt_num]
        # linear_sum_assignment 接收一个成本矩阵，然后返回两组索引：一组是行索引，另一组是列索引。这些索引配对起来，正好对应于矩阵中最小总和的那些元素。
